...
import SharedCalendarYear from '@/components/SharedCalendarYear';
...
interface SharedCalendarYearData {
  trade: CalendarYearData;
  orders: any[];
  metadata: {
    isCalendarYearShare: boolean;
    year: number;
  };
  expiresAt: string;
  createdAt: string;
...
export default function SharedCalendarYearPage() {
  const params = useParams();
  const { key } = params;

  const [data, setData] = useState<SharedCalendarYearData | null>(null);
  ...
  useEffect(() => {
    const fetchSharedCalendar = async () => {
      try {
        ...
        // Validate that this is a calendar year share
        if (!result.isCalendarYearShare) {
          throw new Error('This link is not for a calendar year view');
        }
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold text-gray-900">
                Calendar Year: {calendarData.year}
              </h1>
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <SharedCalendarYear
          ...
          yearData={calendarData.yearData}
          yearStats={calendarData.yearStats}
        />
