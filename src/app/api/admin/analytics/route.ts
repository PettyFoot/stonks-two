...
export async function GET(request: NextRequest) {
  try {
    ...
    const dailySessions = await prisma.$queryRaw<Array<{ date: string; count: bigint }>>`
      SELECT
    ...
    const journeyPaths = await prisma.$queryRaw<Array<{ path: string; count: bigint }>>`
      SELECT
        CONCAT("landingPage", ' → ', STRING_AGG("AnalyticsPageView"."normalizedPath", ' → ' ORDER BY "enteredAt")) as path,
        COUNT(DISTINCT "AnalyticsSession"."sessionId")::int as count
      FROM "AnalyticsSession"
      LEFT JOIN "AnalyticsPageView" ON "AnalyticsSession"."sessionId" = "AnalyticsPageView"."sessionId"
      GROUP BY "AnalyticsSession"."sessionId", "landingPage"
      HAVING COUNT("AnalyticsPageView"."id") > 0
      ORDER BY count DESC
      LIMIT 20
    `;
